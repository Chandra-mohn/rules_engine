...
/**
 * Generated rule class for: complexBusinessDateRule
 * 
 ...
 * 
 * Generated on: 2025-08-26T09:47:34.743281
...
public class ComplexBusinessDateRuleRule implements Rule {
    
    private static final String RULE_NAME = "complexBusinessDateRule";
    ...
    public RuleResult execute(RuleContext context) {
        ...
        if ((compareNumbers(context.getValue("applicant.creditScore"), 700) > 0 && compareNumbers(context.getValue("applicant.applicationDate"), getBusinessDate()) >= 0)) {
            actions.add("approveApplication");
        }
        
        // Step: Step[condition=AndExpression[operands=2], then=Action[rejectApplication]]
        if ((compareNumbers(context.getValue("applicant.creditScore"), 700) <= 0 && compareNumbers(context.getValue("applicant.applicationDate"), getBusinessDate()) < 0)) {
