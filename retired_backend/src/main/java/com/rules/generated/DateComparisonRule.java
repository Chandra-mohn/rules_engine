...
/**
 * Generated rule class for: dateComparison
 * 
 ...
 * 
 * Generated on: 2025-08-26T09:47:34.175554
...
public class DateComparisonRule implements Rule {
    
    private static final String RULE_NAME = "dateComparison";
    ...
    public RuleResult execute(RuleContext context) {
        ...
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=Attribute[applicant.birthDate], operator=before, right=Value[type=STRING, value=2000-01-01]]], then=Action[approveApplication]]
        if (isDateBefore(context.getValue("applicant.birthDate"), "2000-01-01")) {
            actions.add("approveApplication");
        }
        
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=Attribute[applicant.applicationDate], operator=after, right=Value[type=STRING, value=2025-01-01]]], then=Action[rejectApplication]]
        if (isDateAfter(context.getValue("applicant.applicationDate"), "2025-01-01")) {
