...
/**
 * Generated rule class for: businessDateCheck
 * 
 ...
 * 
 * Generated on: 2025-08-26T09:47:34.619590
...
public class BusinessDateCheckRule implements Rule {
    ...
    public RuleResult execute(RuleContext context) {
        ...
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=Attribute[applicant.applicationDate], operator=after, right=DateTimeLiteral[BUSINESS_DATE:BUSINESS_DATE]]], then=Action[conditionalApproval]]
        if (isDateAfter(context.getValue("applicant.applicationDate"), getBusinessDate())) {
            actions.add("conditionalApproval");
        }
        
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=Attribute[applicant.applicationDate], operator=before, right=DateTimeLiteral[BUSINESS_DATE:BUSINESS_DATE]]], then=Action[rejectApplication]]
        if (isDateBefore(context.getValue("applicant.applicationDate"), getBusinessDate())) {
