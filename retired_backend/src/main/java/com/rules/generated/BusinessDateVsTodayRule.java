...
/**
 * Generated rule class for: businessDateVsToday
 * 
 ...
 * 
 * Generated on: 2025-08-26T09:47:34.702497
...
public class BusinessDateVsTodayRule implements Rule {
    
    private static final String RULE_NAME = "businessDateVsToday";
    ...
    public RuleResult execute(RuleContext context) {
        ...
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=DateTimeLiteral[BUSINESS_DATE:BUSINESS_DATE], operator==, right=DateTimeLiteral[TODAY:TODAY]]], then=Action[approveApplication]]
        if (Objects.equals(getBusinessDate(), getTodayAtMidnight())) {
            actions.add("approveApplication");
        }
        
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=DateTimeLiteral[BUSINESS_DATE:BUSINESS_DATE], operator=before, right=DateTimeLiteral[TODAY:TODAY]]], then=Action[manualReview]]
        if (isDateBefore(getBusinessDate(), getTodayAtMidnight())) {
