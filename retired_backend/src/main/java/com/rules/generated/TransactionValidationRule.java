...
/**
 * Generated rule class for: transactionValidation
 * 
 ...
 * 
 * Generated on: 2025-08-26T09:47:34.518741
...
public class TransactionValidationRule implements Rule {
    
    private static final String RULE_NAME = "transactionValidation";
    ...
    public RuleResult execute(RuleContext context) {
        ...
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=Attribute[transaction.amount], operator=>, right=Attribute[card.availableBalance]]], then=Action[blockTransaction]]
        if (compareNumbers(context.getValue("transaction.amount"), context.getValue("card.availableBalance")) > 0) {
            actions.add("blockTransaction");
        }
        
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=Attribute[transaction.amount], operator=<=, right=Attribute[card.availableBalance]]], then=Action[approveTransaction]]
        if (compareNumbers(context.getValue("transaction.amount"), context.getValue("card.availableBalance")) <= 0) {
