...
/**
 * Generated rule class for: ageCheck
 * 
 ...
 * 
 * Generated on: 2025-08-26T09:47:34.354215
...
public class AgeCheckRule implements Rule {
    ...
    private static final String RULE_NAME = "ageCheck";
    ...
    public RuleResult execute(RuleContext context) {
        ...
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=ArithmeticExpression[operands=2, operators=1], operator=>=, right=Value[type=NUMBER, value=18]]], then=Action[approveApplication]]
        if (compareNumbers((evaluateArithmetic(getYear(getCurrentDateTime()), "-", getYear(context.getValue("applicant.birthDate")))), 18) >= 0) {
            actions.add("approveApplication");
        }
        
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=ArithmeticExpression[operands=2, operators=1], operator=<, right=Value[type=NUMBER, value=18]]], then=Action[rejectApplication]]
        if (compareNumbers((evaluateArithmetic(getYear(getCurrentDateTime()), "-", getYear(context.getValue("applicant.birthDate")))), 18) < 0) {
