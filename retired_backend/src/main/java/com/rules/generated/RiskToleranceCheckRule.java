...
/**
 * Generated rule class for: riskToleranceCheck
 * 
 ...
 * 
 * Generated on: 2025-08-26T09:47:35.624846
...
public class RiskToleranceCheckRule implements Rule {
    
    private static final String RULE_NAME = "riskToleranceCheck";
    ...
    public RuleResult execute(RuleContext context) {
        ...
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=Attribute[merchant.riskScore], operator=>, right=Attribute[customer.riskTolerance]]], then=Action[requireVerification]]
        if (compareNumbers(context.getValue("merchant.riskScore"), context.getValue("customer.riskTolerance")) > 0) {
            actions.add("requireVerification");
        }
        
        // Step: Step[condition=AndExpression[operands=2], then=Action[requireStepUpAuth]]
        if ((compareNumbers(context.getValue("transaction.timestamp"), (evaluateArithmetic(context.getValue("card.lastUsedTimestamp"), "+", 86400))) > 0 && compareNumbers(context.getValue("transaction.amount"), context.getValue("customer.dormancyThreshold")) > 0)) {
            actions.add("requireStepUpAuth");
        }
