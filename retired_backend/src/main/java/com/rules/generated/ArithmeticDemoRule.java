...
/**
 * Generated rule class for: arithmeticDemo
 * 
 ...
 * 
 * Generated on: 2025-08-26T09:47:34.082624
...
public class ArithmeticDemoRule implements Rule {
    
    private static final String RULE_NAME = "arithmeticDemo";
    ...
    public RuleResult execute(RuleContext context) {
        ...
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=Attribute[applicant.annualIncome], operator=<, right=MultiplicativeExpression[operands=2, operators=1]]], then=Action[rejectApplication]]
        if (compareNumbers(context.getValue("applicant.annualIncome"), evaluateArithmetic(context.getValue("applicant.monthlyExpenses"), "*", 12)) < 0) {
            actions.add("rejectApplication");
        }
        
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=Attribute[applicant.totalDebt], operator=>, right=MultiplicativeExpression[operands=2, operators=1]]], then=Action[requireManualReview]]
        if (compareNumbers(context.getValue("applicant.totalDebt"), evaluateArithmetic(context.getValue("applicant.annualIncome"), "*", 0.4)) > 0) {
            actions.add("requireManualReview");
        }
        
        // Step: Step[condition=NotExpression[negated=false, operand=Comparison[left=Attribute[transaction.amount], operator=>, right=MultiplicativeExpression[operands=2, operators=1]]], then=Action[flagForReview]]
        if (compareNumbers(context.getValue("transaction.amount"), evaluateArithmetic(context.getValue("customer.averageTransactionAmount"), "*", 5)) > 0) {
            actions.add("flagForReview");
        }
